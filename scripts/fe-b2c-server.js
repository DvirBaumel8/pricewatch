...
/**
 * Thin FE-B2C — static UI + optional reverse-proxy to Service A.
 *
 * Serves public/fe-b2c/ only. Browser JS calls Service A HTTP APIs
 * (same-origin via proxy, or SERVICE_A_URL directly). No second backend:
 * this process never imports product-offer / slot / watch-target stores.
 *
 * Usage:
 *   npm run fe-b2c
 *   node scripts/fe-b2c-server.js [--port 3921]
 *
 * Env:
 *   SERVICE_A_URL   default http://127.0.0.1:3850
 *   FE_B2C_PORT     default 3921 (or --port)
 *   FE_B2C_HOST     default 127.0.0.1
 *
 ...
 * Production/pilot path: Service A hosts /fe-b2c/ (Wave 9) — no new paid service.
 */
...
const STATIC_ROOT = path.join(PROJECT_ROOT, "public", "fe-b2c");

const PORT = parseInt(
  process.argv.find((_, i, a) => a[i - 1] === "--port") ||
    process.env.FE_B2C_PORT ||
    "3921",
  10
);
const HOST = process.env.FE_B2C_HOST || "127.0.0.1";
...
function isServiceAPath(pathname) {
  return (
    ...
    pathname === "/product-offers" ||
    pathname.startsWith("/product-offers/") ||
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" || req.method === "HEAD") {
    if (pathname === "/fe-b2c" || pathname === "/fe-b2c/") {
      serveStatic(req, res, "/");
      return;
    }
...
if (require.main === module) {
  if (!fs.existsSync(path.join(STATIC_ROOT, "index.html"))) {
    console.error("[fe-b2c] missing public/fe-b2c/index.html");
    process.exit(1);
  }
  server.listen(PORT, HOST, () => {
    console.log(`[fe-b2c] static UI on http://${HOST}:${PORT}/`);
    console.log(`[fe-b2c] proxy → Service A at ${SERVICE_A_URL}`);
    console.log(
      "[fe-b2c] Start Service A first. AUTH_STUB=1 is local/shame only — never prod."
    );
    console.log("[fe-b2c] Live Google OAuth: PARKED (Wave 6 Phase B).");
    console.log("[fe-b2c] Hosted path on Service A: /fe-b2c/ (no new paid service).");
  });
}
